...
p=Path('backend/test-output/industry40-stack/5d4cd2636806/receipt.json')
...
old=inspect(name)
assert name=='nis-e2e-app-5d4cd2636806' and old['Config']['Labels']['networkis.test']=='disposable'
...
Path('.tool-checker/ai-preflight.json').write_text(json.dumps({'models':models,'test_container':name,'base_url':r['base_url'],'image_id':r['image_id']},indent=2),encoding='utf-8')
