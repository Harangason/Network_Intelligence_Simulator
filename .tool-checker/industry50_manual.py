...
def snapshot(cid,label):
 p='nis-e2e-industry50-'+cid.lower();d=ROOT/'.tool-checker/evidence/industry50'/cid;d.mkdir(parents=True,exist_ok=True)
 value={}
 for key,path in [('workflow','/api/engineering/workflow?view=summary'),('history','/api/agent/history?projectId='+p),('conversation','/api/engineering/agent/conversation'),('hardware','/api/engineering/hardware-nodes'),('hardware_interfaces','/api/engineering/hardware-interfaces'),('functions','/api/engineering/functions'),('messages','/api/engineering/messages'),('signals','/api/engineering/signals'),('routing','/api/engineering/routing'),('resources','/api/engineering/communication-resources')]:
  code,body=request('http://127.0.0.1:60023',p,path)
...
def begin(cid):
 c=next(c for c in M['test_cases'] if c['test_id']==cid);d=ROOT/'.tool-checker/evidence/industry50'/cid
 before=snapshot(cid,'before');plan=cli('tool_check.py','--task','industry50','dry-run',cid)[0];assert not plan['blockers'],plan['blockers']
 proof={'source_hash':plan['context']['source_hash'],'contract_hash':plan['context']['test']['contract_hash'],'project_path':str(ROOT),'checked_at':datetime.now(timezone.utc).isoformat(),'available_skills':['tool-checker'],'available_tools':['NIS HTTP API','Browser'],'available_views':['Engineering'],'browser':{'observed_working':True},'isolated_scope':'Disposable published image at port60023; nis-e2e-industry50 only'}
 ...
 out=cli('tool_check.py','--task','industry50','run',cid,'--preflight',str(d/'preflight.json'),'--baseline',str(d/'before.json'));(d/'manual-run.json').write_text(json.dumps(out,indent=2));print(json.dumps(out))
