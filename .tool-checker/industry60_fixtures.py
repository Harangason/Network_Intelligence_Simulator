...
BASE='http://127.0.0.1:51576'
def api(project,path,data=None,method=None):
    assert project.startswith('nis-e2e-industry60-')
...
def seed(project):
    ...
    folder=ROOT/'.tool-checker/evidence/industry60'/project.rsplit('-',1)[-1].upper();folder.mkdir(parents=True,exist_ok=True)
    (folder/'fixture.json').write_text(json.dumps(data,indent=2),encoding='utf-8')
    return data
if __name__=='__main__':
    import sys
    print(json.dumps(seed('nis-e2e-industry60-'+sys.argv[1].lower()),indent=2))
