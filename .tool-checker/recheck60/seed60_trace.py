...
OUT=Path('.tool-checker/evidence/industry60-recheck/trace');P='nis-e2e-industry60-trace';mapping={};created={}
cfg=json.loads((OUT/'golden-config.json').read_text())
...
for label in ['golden','fault']:
    original=json.loads((OUT/(label+'-config.json')).read_text());result=remap(original)
