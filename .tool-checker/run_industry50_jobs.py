...
def setup():
    cfg=json.loads((STATE/'config.json').read_text());cfg.setdefault('adapters',{})['industry50-http']={
        'argv':[sys.executable,str(ROOT/'.tool-checker/industry50_adapter.py')],'uses_llm':True,'mutating':True}
    (STATE/'config.json').write_text(json.dumps(cfg,indent=2),encoding='utf-8')
    path=ROOT/'.tool-checker/industry50.normalized.json';m=json.loads(path.read_text(encoding='utf-8'))
    for c in m['test_cases'][:40]:
        ...
        c['execution_plan']=[{'step_id':'real-agent','step_label':'Originalauftrag und Persistenz prüfen',
            'kind':'adapter','adapter':'industry50-http','channel':'mcp','phase':'engineering_execution',
            'base_url':'http://127.0.0.1:60023','project_id':'nis-e2e-industry50-'+c['test_id'].lower()+'-'+uuid.uuid4().hex[:8],'case':bare}]
    path.write_text(json.dumps(m,ensure_ascii=False,indent=2),encoding='utf-8')
    print(cli('tool_check.py','ingest','H:/OneDrive/Download/NETWORK_SIMULATOR_50_INDUSTRY_NEUTRAL_AGENT_MCP_TEST_SCENARIOS.md','--id','industry50','--normalized',str(path)))
def run():
    m=json.loads((ROOT/'.tool-checker/industry50.normalized.json').read_text(encoding='utf-8'))
    selected=sys.argv[2:]
    for case in m['test_cases'][:40]:
        if selected and case['test_id'] not in selected: continue
        directory=ROOT/'.tool-checker/evidence/industry50'/case['test_id'];directory.mkdir(parents=True,exist_ok=True)
        if (directory/'job.json').exists(): continue
        step=case['execution_plan'][0];base=step['base_url'];project=step['project_id']
        plan=cli('tool_check.py','--task','industry50','dry-run',case['test_id'])[0]
        ...
        proof={'source_hash':plan['context']['source_hash'],'contract_hash':plan['context']['test']['contract_hash'],
            ...
            'dependencies':{'status':'PASSED','evidence':'Isolated production image b1297e9bad56; model tags in industry50-ai-preflight.json'},
            ...
            'permissions':{'status':'PASSED','evidence':'User requested rerun of all 50 scenarios in disposable stack'},
            ...
            'isolated_scope':'nis-e2e-app-96f25e6a865f; disposable SQL and runtime; no product writes'}
        ...
        job=cli('tool_jobs.py','submit','--task','industry50','--test',case['test_id'],'--preflight',str(directory/'preflight.json'),
