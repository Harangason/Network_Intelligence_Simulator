...
def setup():
    cfg=json.loads((STATE/'config.json').read_text());cfg.setdefault('adapters',{})['industry60-http']={
        'argv':[sys.executable,str(ROOT/'.tool-checker/industry60_adapter.py')],'uses_llm':True,'mutating':True}
    (STATE/'config.json').write_text(json.dumps(cfg,indent=2),encoding='utf-8')
    path=ROOT/'.tool-checker/industry60.normalized.json';m=json.loads(path.read_text(encoding='utf-8'))
    for c in m['test_cases'][:40]:
        ...
        c['execution_plan']=[{'step_id':'real-agent','step_label':'Originalauftrag und Persistenz prüfen',
            'kind':'adapter','adapter':'industry60-http','channel':'mcp','phase':'engineering_execution',
            'base_url':'http://127.0.0.1:51576','project_id':'nis-e2e-industry60-'+c['test_id'].lower()+'-'+uuid.uuid4().hex[:8],'case':bare}]
    path.write_text(json.dumps(m,ensure_ascii=False,indent=2),encoding='utf-8')
    print(cli('tool_check.py','ingest','H:/OneDrive/Download/NETWORK_SIMULATOR_60_INDUSTRY_NEUTRAL_AGENT_MCP_TRACE_TEST_SCENARIOS.md','--id','industry60','--normalized',str(path)))
def run():
    m=json.loads((ROOT/'.tool-checker/industry60.normalized.json').read_text(encoding='utf-8'))
    selected=sys.argv[2:]
    for case in m['test_cases'][:40]:
        if selected and case['test_id'] not in selected: continue
        directory=ROOT/'.tool-checker/evidence/industry60'/case['test_id'];directory.mkdir(parents=True,exist_ok=True)
        if (directory/'job.json').exists(): continue
        step=case['execution_plan'][0];base=step['base_url'];project=step['project_id']
        plan=cli('tool_check.py','--task','industry60','dry-run',case['test_id'])[0]
        ...
        proof={'source_hash':plan['context']['source_hash'],'contract_hash':plan['context']['test']['contract_hash'],
            ...
            'dependencies':{'status':'PASSED','evidence':'Isolated production image c16f447ee305; model tags in industry60-ai-preflight.json'},
            ...
            'isolated_scope':'nis-e2e-app-f4a32bef96e8; disposable SQL and runtime; no product writes'}
        ...
        job=cli('tool_jobs.py','submit','--task','industry60','--test',case['test_id'],'--preflight',str(directory/'preflight.json'),
