...
OUT=Path(__file__).resolve().parent/'evidence/industry50/mcp'
...
def test_observe_registered_protocol_and_failures():
    ...
    async def run():
        output={}; server=create_server(ToolAuthority('nis-e2e-industry50-mcp-'+uuid4().hex))
        async with EngineeringMCPClient(server) as client:
            ...
            calls=[('unknown','nonexistent_industry50_capability',{}),
                ('invalid','calculate_message_size',{'technology':'CAN_FD','payload_bytes':-2}),
                ('unsupported','calculate_message_size',{'technology':'INDUSTRY50_UNKNOWN','payload_bytes':1}),
