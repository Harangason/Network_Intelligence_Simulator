...
from ..orchestration.capability_intent import connection_request, explicit_trace_analysis, signal_inspection
...
def adapt_input(prompt, context, *, run_id, raw_input=None, operation_id=None, revision=None):
    ...
    if connection_request(prompt):
        intent = 'CONNECT_FUNCTIONS'
    elif context.requested_mode:
        intent = context.requested_mode
    elif explicit_trace_analysis(prompt):
        intent = 'ANALYZE_TRACE'
    elif signal_inspection(prompt):
        intent = 'VALIDATE_SIGNAL'
    else:
        intent = 'CREATE_ARCHITECTURE' if is_project_request(prompt) else 'ENGINEERING_REQUEST'
