...
export async function createLocalSimulation(rawPayload: Record<string, unknown>, validateOnly: boolean): Promise<SimulationJob> {
  ...
  const formats = Array.isArray(config.formats) ? config.formats.map(String) : defaultSimulationFormats;
  ...
  const artifacts = validateOnly ? [] : formats.map((format, index) => format === "universal-csv" || format === "csv"
    ? download(`${id}.csv`, `timestamp_ms,source,target,technology,payload_bytes,valid\n${rows.map((row) => Object.values(row).join(",")).join("\n")}`, index)
    : download(`${id}.${simulationFormatExtension(format)}`, rows.map((row) => JSON.stringify(row)).join("\n"), index));
