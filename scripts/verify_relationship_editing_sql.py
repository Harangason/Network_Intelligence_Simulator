...
database = 'nis_relationship_editing_tests'
...
with psycopg.connect(urlunsplit(url._replace(path='/postgres')), autocommit=True) as connection:
    if not connection.execute('SELECT 1 FROM pg_database WHERE datname=%s', (database,)).fetchone():
        connection.execute('CREATE DATABASE nis_relationship_editing_tests')
...
result = pytest.main(['backend/tests/test_relationship_editing.py', 'backend/tests/test_port_attachment.py', 'backend/tests/test_topology_removal.py', '-q', '-p', 'no:cacheprovider', '--basetemp', '/tmp/pytest-relationship-editing'])
